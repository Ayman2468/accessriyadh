<template>
<div v-if="applications.data && applications.data[0]" class="accordion container my-3" id="accordionExample">
  <div class="accordion-item" v-for="application in applications.data">
    <h2 class="accordion-header" id="headingOne">
      <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" :data-bs-target="'#collapse'+application.id" aria-expanded="false" aria-controls="collapseOne">
        {{ __('audit.Application Information') }} #{{ application.id }} - {{application.building_or_business_name}}
      </button>
    </h2>
    <div :id="'collapse'+application.id" class="accordion-collapse collapse" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
            ...
            <div class="content">
                ...
                <div class="row mt-2">
                    <div v-for="(item,index) in application.answers" class="col-lg-12 col-md-12 mb-3">
                        ...
                        <div class="fw-bold" v-if="lang == 'ar'">{{ item.answer_ar }}</div>
                        <div class="fw-bold" v-else>{{ item.answer }}</div>
                    </div>
                </div>
            </div>
        </div>
...
<div v-else class="d-flex align-items-center justify-content-center vh-100">
    <div class="my-3">
        <p class="alert alert-light h3">{{ __('audit.No Completed Applications Found') }}</p>
    </div>
</div>
</template>
