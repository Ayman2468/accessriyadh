<template>
    <!-- delete-service -->
    <div class="modal fade " id="edit-question" tabindex="-1" aria-labelledby=" edit-serviceLabel"
...
export default {
    ...
    async mounted() {
        emitter.$on('question-edit', (data) => {
            this.form = data.item
            this.title_items = data.title_items
    ...
    methods: {
        update_item(e) {
            ...
            axios.post('/admin/building-type/' + (this.$route.params.id) + '/questions/' + (this.form.question_id) + '/update', formData).then((response) => {
                this.loading = false;
