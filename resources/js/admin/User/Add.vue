<template>
    <!-- add-client -->
    <div class="modal fade modal-custom" id="add-sub-admin" tabindex="-1" aria-labelledby="add-serviceLabel"
         aria-hidden="true"
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <header class="d-flex justify-content-between align-items-start p-4 mb-5 fixed-top bg-white"
                            ...
                            <a href="#" class="close-modal-btn" data-bs-dismiss="modal" @click="close_modal"><img :src="'/images/close.svg'"></a>
                        ...
                        <footer class="d-flex justify-content-between align-items-start p-4 mt-5 fixed-bottom bg-white"
                            ...
                            <div class="d-flex">
                                <a href="#" @click="store_item"
                                   class="btn btn-primary">{{ __('general.Save') }}</a>
                            </div>
                        </footer>
...
export default {
    ...
    async mounted() {
        ...
        this.$root.$on('add_new_item', function (lists) {
    ...
    methods: {
        ...
        create_item() {
            ...
            axios.get('/vue/admin/user/create').then((response) => {
        ...
        store_item(e) {
            ...
            if (this.item.role)
                formData.append('role', this.item.role.key);
            axios.post(
                '/vue/admin/user/store',
                formData, config).then((response) => {

                this.notify('success', 'تم اضافة مستخدم جديد بنجاح', 'fas fa-check p-1');
                this.item = {
                    identity: null,
                    name: null,
                    mobile_country_code: 966,
        ...
        onImageChange(e) {
            ...
            document.getElementById('clientPreview').src = this.item.image_url;
        },
